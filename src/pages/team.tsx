...
const Candidatos: React.FC = () => {
  return (
      <div className="grid sm:grid-cols-1 lg:grid-cols-3 2xl:grid-cols-3 gap-4 my-8 mx-4">
        {candidatos.CandidatosECOA.map(
          (candidato: Candidato, index: number) => (
            <Link key={index} to={`/candidato/${candidato.name}`}>
              <div
                className="transition-all cursor-pointer border rounded flex md:flex-row 
           hover:bg-zinc-200/40 md:items-start"
              >
                ...
                <div className="flex-col grid justify-center max-sm:mx-4 sm:mx-2 lg:mx-0">
                  <h3 className="text-xl sm:text-lg font-semibold text-zinc-950">
                    {candidato.name}
                  </h3>
                  <p className="text-zinc-950 line-clamp-4 sm:line-clamp-none lg:line-clamp-4 break-words mr-2 max-sm:text-xs sm:text-xs lg:text-base">
                    {candidato.preview}
                  </p>
                </div>
              </div>
